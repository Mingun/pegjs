...
function generateJS(ast, options) {
  ...
  function generateTables() {
    ...
    function buildExpectation(e) {
      switch (e.type) {
        case "rule": {
          return "peg$ruleExpectation(\"" + js.stringEscape(e.value) + "\")";
        }
        case "literal": {
  ...
  function generateRuleHeader(ruleNameCode, ruleIndexCode) {
    ...
    if (options.cache) {
      parts.push([
        "var key = peg$offset * " + ast.rules.length + " + " + ruleIndexCode + ";",
        "var cached = peg$resultsCache[key];",
        "",
        "if (cached) {",
        "  peg$offset = cached.nextPos;",
        ""
      ].join("\n"));
      ...
      if (options.trace) {
        parts.push([
          ...
          "    location: peg$computeLocation(startPos, peg$offset)",
          "  });",
          "} else {",
          "  peg$tracer.trace({",
          "    type: \"rule.fail\",",
  ...
  function generateRuleFooter(ruleNameCode, resultCode) {
    ...
    if (options.cache) {
      parts.push([
        "",
        "peg$resultsCache[key] = { nextPos: peg$offset, result: " + resultCode + " };"
      ].join("\n"));
    }
  ...
  function generateRuleFunction(rule) {
      ...
      while (ip < end) {
        switch (bc[ip]) {
          ...
          case op.PUSH_CURR_POS:      // PUSH_CURR_POS
            parts.push(posStack.push("peg$offset"));
            ip++;
            break;

          case op.LOAD_CURR_POS:      // LOAD_CURR_POS
            parts.push("peg$offset = " + posStack.top() + ";");
            ip++;
            break;
          ...
          case op.TEXT:               // TEXT
            resStack.pop();
            parts.push(
              resStack.push("input.substring(" + posStack.top() + ", peg$offset)")
            );
            ip++;
            break;
          ...
          case op.MATCH_ANY:          // MATCH_ANY a, f, ...
            compileCondition("input.length > peg$offset", 0);
            break;

          case op.MATCH_STRING:       // MATCH_STRING s, a, f, ...
            compileCondition(
              ast.literals[bc[ip + 1]].length > 1
                ? "input.substr(peg$offset, "
                    + ast.literals[bc[ip + 1]].length
                    + ") === "
                    + l(bc[ip + 1])
                : "input.charCodeAt(peg$offset) === "
          ...
          case op.MATCH_CLASS:        // MATCH_CLASS c, a, f, ...
            compileCondition(
              r(bc[ip + 1]) + ".test(input.charAt(peg$offset))",
              1
            );
            break;
          ...
          case op.ACCEPT_N:           // ACCEPT_N n
            parts.push(resStack.push(
              bc[ip + 1] > 1
                ? "input.substr(peg$offset, " + bc[ip + 1] + ")"
                : "input.charAt(peg$offset)"
            ));
            parts.push(
              bc[ip + 1] > 1
                ? "peg$offset += " + bc[ip + 1] + ";"
          ...
          case op.ACCEPT_STRING:      // ACCEPT_STRING s
            ...
            parts.push(
              ast.literals[bc[ip + 1]].length > 1
                ? "peg$offset += " + ast.literals[bc[ip + 1]].length + ";"
                : "peg$offset++;"
            );
            ip += 2;
            break;
          ...
          case op.LOAD_SAVED_POS:     // LOAD_SAVED_POS
            parts.push("peg$mark = " + posStack.top() + ";");
            ip++;
            break;

          case op.UPDATE_SAVED_POS:   // UPDATE_SAVED_POS
            parts.push("peg$mark = peg$offset;");
            ip++;
            break;
    ...
    if (options.trace) {
      parts.push("  var startPos = peg$offset;");
    }
  ...
  function generateToplevel() {
    ...
    parts.push([
      ...
      "    },",
      "",
      "    rule: function(expectation) {",
      "      return expectation.description;",
      "    },",
      "",
      "    user: function(expectation) {",
      "      return expectation.description;",
      "    },",
      "",
    ...
    parts.push([
      ...
      "",
      "  function range() {",
      "    return [peg$mark, peg$offset];",
      "  }",
      "",
      "  function location() {",
      "    return peg$computeLocation(peg$mark, peg$offset);",
      "  }",
      "",
      "  function text() {",
      "    return input.substring(peg$mark, peg$offset);",
      "  }",
      "",
      "  function expected(description, loc) {",
      "    throw peg$buildStructuredError(",
      "      [{ type: 'user', description: description }],",
      "      text(),",
      "      loc !== undefined ? loc : location()",
      "    );",
      "  }",
      "",
      ...
      "",
      "  function peg$begin() {",
      "    peg$expected.push({ offset: peg$offset, variants: [] });",
      "  }",
      "",
      "  function peg$expect(expected) {",
      "    var top = peg$expected[peg$expected.length - 1];",
      "",
      "    if (peg$offset < top.offset) { return; }",
      "",
      "    if (peg$offset > top.offset) {",
      "      top.offset = peg$offset;",
      "      top.variants = [];",
      "    }",
      "",
    ...
    parts.push([
      ...
      "",
      "  if (peg$result !== peg$FAILED && peg$offset === input.length) {",
      "    return peg$result;",
      "  }",
      "",
      "  if (peg$result !== peg$FAILED && peg$offset < input.length) {",
  ...
  function generateWrapper(toplevelCode) {
    function generateGeneratedByComment() {
      return [
        "// Generated by PEG.js " + require("../../peg").VERSION,
        "//",
        "// https://pegjs.org/"
      ].join("\n");
    }
