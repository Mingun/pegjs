...
function generateJS(ast, options) {
  ...
  function generateTables() {
    if (options.optimize === "size") {
      return [
        ...
        "];",
        "var peg$ruleParamCounts = [",
        indent2(ast.rules.map(rule => rule.params.length).join(", ")),
        "];"
      ].join("\n");
    } else {
  ...
  function generateInterpreter() {
    ...
    function generateRule() {
      let argCountCode = "peg$ruleParamCounts[bc[ip + 1]]";
      ...
      return [
        ...
        "}",
        "",
        "stack.push(peg$parseRule(bc[ip + 1], bc.slice(ip + 2, ip + 2 + offset)));",
        "ip += 2 + offset;",
        "",
        "break;"
      ].join("\n");
    ...
    function generateParamRef() {
      ...
      let paramIndexCode = "bc[ip + 1]";

      return [
        "offset = peg$ruleParamCounts[ruleIndex];",
        // Find offset for begin of bytecode for parameter <p>;
        "for (i = 0; i < " + paramIndexCode + "; ++i) {",
        "  offset += argsBC[i];",
        "}",
        "",
        "stack.push(peg$parse(ruleIndex, argsBC, offset, offset + argsBC[" + paramIndexCode + "], null));",
        "ip += 2;",
        "",
        "break;"
      ].join("\n");
    ...
    parts.push([
      ...
      "}",
      "",
      "function peg$parseRule(index, argsBC) {"
    ].join("\n"));
    ...
    parts.push("  result = peg$parse(index, bc, 0, bc.length, argsBC);");

    parts.push(indent2(generateRuleFooter("peg$ruleNames[index]", "result")));

    parts.push([
      "}",
      "",
      "function peg$parse(ruleIndex, bc, ip, end, argsBC) {",
      "  var ips   = [];",
      "  var ends  = [];",
      "  var stack = [];",
      "  var params;",
  ...
  function r(r) { return "peg$parse" + r.name; } // parse rule function name

  function generateRuleFunction(rule) {
    return generateParseFunction(
      r(rule), rule.params, rule.bytecode, rule
    );
  }

  function generateParseFunction(name, params, bytecode, rule) {
    let parts = [];
    let stackVars = [];
    let code;
    ...
    function compile(bc) {
      ...
      function compileRule() { // RULE r (len|cnt| bc|cnt|)? cnt = params.length
        let rule = ast.rules[bc[ip + 1]];
        let argCount = rule.params.length;
        let summaryLen = argCount;

        for (let i = 0; i < argCount; ++i) {
          let len   = bc[ip + 2 + i];
          let begin = ip + 2 + summaryLen;

          parts.push(stack.push(generateParseFunction(
            params[i], [], bc.slice(begin, begin + len), null
          )));
          summaryLen += len;
        }
        let args = stack.pop(argCount).join(", ");
        parts.push(stack.push(r(rule) + "(" + args + ")"));
        ip += 2 + summaryLen;
      }
      ...
      while (ip < end) {
        switch (bc[ip]) {
          ...
          case op.PARAM_REF:          // PARAM_REF p
            parts.push(stack.push(p(params[bc[ip + 1]]) + "()"));
            ip += 2;
            break;
    ...
    parts.push("function " + name + "(" + params.map(p).join(", ") + ") {");
