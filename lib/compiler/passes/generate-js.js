...
function generateJS(ast, options) {
  // API variables and functions, available to the user code
  const apis = "input, options, text, offset, range, location, expected, error";

  // These only indent non-empty lines to avoid trailing whitespace.
  const lineMatchRE = /^([^`\r\n]+?(?:`[^`]*?`[^\r\n]*?)?)$/gm;
  function indent2(code) { return code.replace(lineMatchRE, "  $1"); }
  function indent4(code)  { return code.replace(lineMatchRE, "    $1");       }
  ...
  function generateSandbox() {
    ...
    let rest  = funcs.slice(0);

    function doGenerateSandbox(initializer, namespace, i) {
      // @todo: May be share all API between all sandboxes?
      parts.push("  function sandbox" + i + "(" + apis + ") {");
    ...
    function generate(initializer, namespace) {
      ...
      rest = rest.filter(f => f.namespace !== namespace);
    }

    parts.push("function peg$sandbox(" + apis + ") {");

    // First, generate all functions with corresponds initializers.
    if (ast.initializers) {
      ast.initializers.forEach(initializer => {
  ...
  function generateToplevel() {
    ...
    parts.push([
      ...
      "  }",
      "",
      "  var peg$functions = peg$sandbox(" + apis + ");",
      "",
    ].join("\n"));
