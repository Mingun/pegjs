...
function generateJS(ast, options) {
  ...
  function generateInterpreter() {
    ...
    function generateCall() {
      ...
      return [
        ...
        "  bc[ip + 2],",
        "  new peg$Result(peg$consts[bc[ip + 1]].apply(null, params.map(peg$getter)))",
        ");",
        "",
    ...
    parts.push([
      ...
      "      switch (bc[ip]) {",
      "        case " + op.PUSH + ":",               // PUSH c
      "          stack.push(new peg$Result(peg$consts[bc[ip + 1]]));",
      "          ip += 2;",
      "          break;",
      "",
      "        case " + op.PUSH_UNDEFINED + ":",     // PUSH_UNDEFINED
      "          stack.push(new peg$Result(undefined));",
      "          ip++;",
      "          break;",
      "",
      "        case " + op.PUSH_NULL + ":",          // PUSH_NULL
      "          stack.push(new peg$Result(null));",
      "          ip++;",
      "          break;",
      "",
      ...
      "",
      "        case " + op.WRAP + ":",               // WRAP n
      "          stack.push(peg$wrap(stack.splice(stack.length - bc[ip + 1], bc[ip + 1])));",
      "          ip += 2;",
      "          break;",
      "",
      "        case " + op.TEXT + ":",               // TEXT
      "          stack.push(peg$text(stack.pop(), stack.pop(), peg$currPos));",
      "          ip++;",
      "          break;",
      "",
      "        case " + op.IF + ":",                 // IF t, f
      indent10(generateCondition("stack[stack.length - 1].get()", 0)),
      "",
      ...
      "          stack.push(new peg$Result(input.substr(peg$currPos, bc[ip + 1])));",
      "          peg$currPos += bc[ip + 1];",
      "          ip += 2;",
      "          break;",
      "",
  ...
  function generateRuleFunction(rule) {
    ...
    let stack = {
      ...
      pushResult(exprCode, lazy) {
        return this.push("new peg$Result(" + exprCode + (lazy ? ", true)" : ")"));
      },
    ...
    function compile(bc) {
      ...
      function compileCall(lazy) {
        let baseLength = 4;
        let paramsLength = bc[ip + baseLength - 1];
        let params = bc.slice(ip + baseLength, ip + baseLength + paramsLength);

        let value = c(bc[ip + 1]) + "("
          + params.map(p => stack.index(p) + ".get()").join(", ")
          + ")";
        if (lazy) {
          params = params.map(p => stack.index(p)).join(", ");
          value = "(function(" + params + ") { return function() { return " + value + "; }; }) (" + params + ")";
        }
        stack.pop(bc[ip + 2]);
        parts.push(stack.pushResult(value, lazy));
      ...
      while (ip < end) {
        switch (bc[ip]) {
          case op.PUSH:               // PUSH c
            parts.push(stack.pushResult(c(bc[ip + 1])));
            ip += 2;
            break;
          ...
          case op.PUSH_UNDEFINED:      // PUSH_UNDEFINED
            parts.push(stack.pushResult("undefined"));
            ip++;
            break;

          case op.PUSH_NULL:          // PUSH_NULL
            parts.push(stack.pushResult("null"));
            ip++;
            break;
          ...
          case op.WRAP:               // WRAP n
            parts.push(
              stack.push("peg$wrap([" + stack.pop(bc[ip + 1]).join(", ") + "])")
            );
            ip += 2;
            break;
          ...
          case op.IF:                 // IF t, f
            compileCondition(stack.top() + ".get()", 0);
            break;
          ...
          case op.CALL:               // CALL f, n, pc, p1, p2, ..., pN
          case op.LAZY_CALL:          // LAZY_CALL f, n, pc, p1, p2, ..., pN
            compileCall(bc[ip] === op.LAZY_CALL);
            break;
  ...
  function generateToplevel() {
    ...
    parts.push([
      ...
      "};",
      "",
      "function peg$Result(value, lazy) {",
      "  this.value = value;",
      "  this.lazy  = lazy || false;",
      "}",
      "",
      "peg$Result.prototype.get = function() {",
      "  if (this.lazy) {",
      "    if (typeof this.value === \"function\") {",
      "      this.value = this.value();",
      "    } else {",
      "      this.value = this.value.map(peg$getter);",
      "    }",
      "    this.lazy = false;",
      "  }",
      "  return this.value;",
      "}",
      "",
      "peg$Result.prototype.push = function(value) {",
      "  if (value.lazy) {",
      "    this.lazy = true;",
      "  }",
      "  return this.value.push(this.lazy ? value : value.get());",
      "}",
      "",
      "function peg$getter(result) {",
      "  return result.get();",
      "}",
      "",
      "function peg$wrap(results) {",
      "  var lazy = results.some(function(r) { return r.lazy; })",
      "  return new peg$Result(lazy ? results : results.map(peg$getter), lazy);",
      "}",
      ""
    ].join("\n"));
    ...
    parts.push([
      ...
      "",
      "  function peg$text(value, savedPos, currPos) {",
      "    if (value.lazy) {",
      "      return new peg$Result(function() { value.get(); return input.substring(savedPos, currPos); }, true);",
      "    }",
      "    return new peg$Result(input.substring(savedPos, currPos));",
      "  }",
      "",
      "  var peg$FAILED = {};",
      ""
