...
function generateJS(ast, options) {
  ...
  function generateInterpreter() {
    ...
    parts.push([
      ...
      "        case " + op.TEXT + ":",               // TEXT
      "          stack.pop();",
      "          stack.push(new peg$Result(input.substring(stack.pop(), peg$currPos)));",
      "          ip++;",
      "          break;",
      "",
      "        case " + op.IF + ":",                 // IF t, f
  ...
  function generateRuleFunction(rule) {
    ...
    let stack = {
      ...
      pushResult(exprCode) {
        return this.push("new peg$Result(" + exprCode + ")");
      },
    ...
    function compile(bc) {
      ...
      function compileCall() {
        ...
        let value = c(bc[ip + 1]) + "("
          ...
          + ")";
        stack.pop(bc[ip + 2]);
        parts.push(stack.pushResult(value));
      ...
      while (ip < end) {
        switch (bc[ip]) {
          ...
          case op.TEXT:               // TEXT
            stack.pop();
            parts.push(
              stack.pushResult("input.substring(" + stack.pop() + ", peg$currPos)")
            );
            ip++;
            break;
  ...
  function generateToplevel() {
    ...
    parts.push([
      ...
      "  return this.value.push(value.get());",
      "}",
      "",
      ...
      "  return new peg$Result(results.map(peg$getter));",
      "}",
      ""
    ].join("\n"));
