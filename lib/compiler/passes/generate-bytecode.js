  ...
  function buildCheckMax(expressionCode, max, context, hasMin) {
    if (max.value !== null) {
      let checkCode = max.constant
        ? [op.IF_GE, max.value]
        : [op.IF_GE_DYNAMIC, context.sp + 1 - context.env[max.value] + (hasMin ? 1 : 0)]; // +1 slot for result array
  ...
  function buildCheckMin(expressionCode, min, context) {
    ...
    if (min.value !== null && (!min.constant || min.value > 0)) {
      let checkCode = min.constant
        ? [op.IF_LT, min.value]
        : [op.IF_LT_DYNAMIC, context.sp + 1 - context.env[min.value] + 1]; // +1 slot for result array
      ...
      return buildSequence(
        ...
        buildCondition(
          ...
          [op.POP, op.POP_CURR_POS,   //   currPos = savedPos;    p: [], r: []
          // eslint-disable-next-line indent-legacy
           op.PUSH_FAILED],           //   result = peg$FAILED;   p: [], r:[ peg$FAILED ]
          [op.POP_POS]                // }                        p: [], r:[ [elem...] ]
        )
      );
    }
    ...
    if (delimiterNode !== null) {
      return buildSequence(               //                          p: [], r:[]
        ...
        generate(delimiterNode, {         // item = delim();          p: [pos], r:[delim]
          sp: context.sp + 1, // +1 slot for result array
          env: Object.assign({}, context.env),
          auto: [],
          action: null,
          reportFailures: context.reportFailures
        ...
        buildCondition(
          ...
          buildSequence(
            ...
            buildCondition(
              ...
              [op.POP,                    //                          p: [pos], r:[]
               op.POP_CURR_POS,           //     peg$currPos = pos;   p: [], r:[]
               op.PUSH_FAILED],           //     item = peg$FAILED;   p: [], r:[FAILED]
  ...
  let generate = visitor.build({
        ...
        ? buildSequence(
            ...
            buildCondition(
              ...
              buildSequence(
                ...
                buildCall(functionIndex, 1, env, context.sp + 2)  // p: [pos], r:[act]
              ),
              []
            ),
            [op.POP_POS]                                          // p: [], r:[act]
    ...
    sequence(node, context) {
        ...
        if (elements.length > 0) {
          ...
          return buildSequence(
            ...
            buildCondition(
              ...
              buildSequence(
                processedCount > 1 ? [op.POP_N, processedCount] : [op.POP], // p: [pos], r:[]
                [op.POP_CURR_POS],                          // p: [], r:[]
                [op.PUSH_FAILED]                            // p: [], r:[FAILED]
              )
            )
          );
        } else {
          ...
          if (resultLen === 0) {
            return buildSequence([op.WRAP, elemsLen], [op.POP_POS]); // p: [], r:[[...]]
          }
          if (resultLen === 1) {
            return buildSequence(                           // p: [], r:[res]
          ...
          return buildSequence(
            ...
            context.auto.map(a => context.sp - a.sp),       // p: [pos], r:[[...]]
            [op.POP_POS]                                    // p: [], r:[[...]]
          );
        }
      }
      ...
      return node.elements.length > 0 ? buildSequence(
        [op.PUSH_CURR_POS],                     // p: [pos], r:[]
        buildElementsCode(node.elements, {      // p: [], r:[???]
          sp: context.sp + 1,
          env: context.env,
          auto: context.auto,
          action: context.action,
    ...
    text(node, context) {
      return buildSequence(
        ...
        buildCondition(
          ...
          [op.IF_NOT_ERROR],
          buildSequence([op.POP], [op.TEXT]),     // p: [], r:[txt]
          [op.POP_POS]                            // p: [], r:[FAILED]
        )
      );
    },
    ...
    range(node, context) {
      ...
      let expressionCode = generate(node.expression, {
        sp: context.sp + 1 + (hasMin ? 1 : 0), // +1 slot for result array
        env: Object.assign({}, context.env),
        auto: [],
        action: null,
        reportFailures: context.reportFailures
      ...
      let mainLoopCode = buildSequence(
        ...
        hasMin ? [op.PUSH_CURR_POS] : [], // var savedPos = curPos;   p: [pos], r:[]
