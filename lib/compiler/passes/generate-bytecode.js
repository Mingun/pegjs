...
function generateBytecode(ast) {
  ...
  function buildCall(functionIndex, delta, env, sp, lazy) {
    let params = Object.keys(env).map(name => sp - env[name]);

    return [
      lazy ? op.LAZY_CALL : op.CALL,
      functionIndex, delta, params.length
    ].concat(params);
  }
  ...
  function buildSemanticPredicate(code, negative, context) {
    ...
    return buildSequence(
      [op.UPDATE_SAVED_POS],
      buildCall(functionIndex, 0, context.env, context.sp, false),
      buildCondition(
        [op.IF],
        buildSequence(
          [op.POP],
      ...
      return emitCall
        ? buildSequence(
            ...
            buildCondition(
              ...
              buildSequence(
                [op.LOAD_SAVED_POS, 1],
                buildCall(functionIndex, 1, env, context.sp + 2, node.lazy)
              ),
              []
            ),
            [op.NIP]
