...
function generateBytecode(ast) {
  ...
  function buildRangeBody(node, sp, context) {
    ...
    let checkMaxCode  = buildCheckMax(node.max, sp, context.env);
    let parseNextCode = buildRangeAppend(match, expressionCode, node.delimiter !== null);
    ...
    if (node.delimiter !== null) {
      ...
      return buildLoop(
        ...
        buildSequence(
          ...
          node.max.constant ? [] : checkMaxCode,
          [op.PUSH_CURR_POS],
          buildLoop(
            [op.LOOP, 1],
  ...
  let generate = visitor.build({
    ...
    range(node, context) {
      ...
      let mainCode = buildRangeBody(node, sp, context);
