  ...
  function buildSimplePredicate(expression, negative, context) {
    ...
    return buildSequence(
      ...
      buildCondition(
        ...
        buildSequence(
          negative ? [op.NIP, op.POP] : [op.POP, op.POP_CURR_POS],
          [op.PUSH_UNDEFINED]
        ),
        buildSequence(
          negative ? [op.POP, op.POP_CURR_POS] : [op.NIP, op.POP],
          [op.PUSH_FAILED]
        )
      )
    );
    ...
    sequence(node, context) {
      function buildElementsCode(elements, context) {
        ...
        if (elements.length > 0) {
          ...
          if (resultLen === 0) {
            return buildSequence([op.WRAP, elemsLen], [op.NIP]);
          }
          if (resultLen === 1) {
            return buildSequence(
