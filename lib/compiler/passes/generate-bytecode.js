...
function generateBytecode(ast) {
  ...
  function buildCheckMax(expressionCode, max) {
    if (max !== null) {
      ...
      return buildCondition(
        0,
        [op.IF_GE, max],  // if (r.length >= max)   stack:[ [elem...] ]
  ...
  function buildCheckMin(expressionCode, min) {
    // When restriction is set and not |0|, check it
    if (min !== null && min > 0) {
      return buildSequence(
        expressionCode,             // result = [elem...];      stack:[ savedPos, [elem...] ]
        buildCondition(
          0,
  ...
  let generate = visitor.build({
    ...
    range(node, context) {
      let hasMin = node.min !== null && node.min > 0;
      let expressionCode = generate(node.expression, {
      ...
      let checkMaxCode = buildCheckMax(expressionCode, node.max);
      ...
      return buildCheckMin(mainLoopCode, node.min);
    },
