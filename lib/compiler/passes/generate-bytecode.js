    ...
    return buildSequence(
      ...
      buildCondition(
        ...
        buildSequence(
          ...
          [negative ? op.POP_POS : op.POP_CURR_POS]
        ),
        buildSequence(
          [op.POP, op.PUSH_FAILED],
          [negative ? op.POP_CURR_POS : op.POP_POS]
        )
      )
    );
  }
    ...
    if (min.value !== null && (!min.constant || min.value > 0)) {
      ...
      return buildSequence(
        ...
        buildCondition(
          ...
          buildSequence(
            [op.POP, op.PUSH_FAILED], //   result = peg$FAILED;   p: [], r:[FAILED]
            [op.POP_CURR_POS]         //   currPos = savedPos;    p: [], r:[ [elem...] ]
          ),
          [op.POP_POS]                // }                        p: [], r:[ [elem...] ]
        )
      );
        ...
        buildCondition(
          ...
          buildSequence(
            ...
            buildCondition(
              ...
              buildSequence(
                [op.POP, op.PUSH_FAILED], //     item = peg$FAILED;   p: [pos], r:[FAILED]
                [op.POP_CURR_POS]         //     peg$currPos = pos;   p: [], r:[FAILED]
              ),
  ...
  let generate = visitor.build({
    ...
    sequence(node, context) {
      ...
      return node.elements.length > 0 ? buildSequence(
        [op.PUSH_CURR_POS],                                 // p: [pos], r:[]
        buildElementsCode(node.elements, context)           // p: [], r:[???]
      ) : [op.PUSH_EMPTY_ARRAY];
    },
