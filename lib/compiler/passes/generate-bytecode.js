...
// [27] RULE r, args
//
//        stack.push(parseRule(r, args));
//
// [34] PARAM_REF p
//
//        stack.push(parseArg(p));
//
// Failure Reporting
// -----------------
//
...
function generateBytecode(ast) {
  ...
  let generate = visitor.build({
    ...
    rule_ref(node, context) {
      let argsBC = node.args.map(function(arg) {
        return generate(arg, {
          sp: context.sp,
          env: cloneEnv(context.env),
          rule: context.rule,
      ...
      let bcLengths = argsBC.map(arg => arg.length);
      let result = [op.RULE, asts.indexOfRule(ast, node.name)];

      Array.prototype.push.apply(result, bcLengths);

      return Array.prototype.concat.apply(result, argsBC);
    },
    ...
    param_ref(node, context) {
      return [op.PARAM_REF, context.rule.params.indexOf(node.name)];
    },
