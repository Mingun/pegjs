...
function generateBytecode(ast) {
  ...
  function buildSemanticPredicate(node, negative, context) {
    let functionIndex = addFunctionConst(Object.keys(context.env), node.code);
  ...
  let generate = visitor.build({
    ...
    named(node, context) {
      ...
      let nameIndex = context.reportFailures ? addConst(
    ...
    sequence(node, context) {
      function buildElementsCode(elements, context) {
        if (elements.length > 0) {
          let processedCount = node.elements.length - elements.length + 1;
    ...
    literal(node, context) {
      if (node.value.length > 0) {
        ...
        let stringIndex = needConst ? addConst("\""
          + js.stringEscape(
        ...
        // Do not generate unused constant, if no need it
        let expectedIndex = context.reportFailures ? addConst(
    ...
    class(node, context) {
      ...
      let parts = "["
        ...
        + "]";
      let match = node.match | 0;
      let regexpIndex = match === 0 ? addConst(regexp) : null;
      // Do not generate unused constant, if no need it
      let expectedIndex = context.reportFailures ? addConst(
    ...
    any(node, context) {
      // Do not generate unused constant, if no need it
      let expectedIndex = context.reportFailures ? addConst("peg$anyExpectation()") : null;
